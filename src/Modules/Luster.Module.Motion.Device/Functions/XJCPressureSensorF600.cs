...
namespace Luster.Module.Motion.Device.Functions
{
    ...
    public class XJCPressureSensorF600 : MotionFunction, IPauseFunction
    {
        ...
        [Parameter("启用多通道模式", 2, CN = "启用多通道", DefaultV = true)]
        public bool IsMultiChEn { get; set; }

        [DependOn("IsMultiChEn", true)]
        [Parameter("通道", 3, CN = "通道", DefaultV = 1)]
        public int ChannelNo { get; set; }

        [Parameter("压力放大器工作模式", 4, CN = "工作模式", DefaultV = PressureActionType.ReadValue)]
        public PressureActionType PActionType { get; set; }

        [DependOn("PActionType", PressureActionType.ReadValue)]
        [Parameter("延时读取压力值", 5, CN = "延时读取", DefaultV = 200)]
        public int DelayTime { get; set; }

        [Parameter("标准压力值", 6, CN = "压力值", DefaultV = 0.5)]
        public double PressureValue { get; set; }

        [Parameter("最小压力值,只有值在大于-1并且value<0才生效", 7, CN = "最小压力值", DefaultV = -1)]
        public double MinValue { get; set; }

        [Range(1, 100)]
        [Parameter("多次读取次数", 8, CN = "多次读取", DefaultV = 1)]
        ...
        public XJCPressureSensorF600()
        {
            this.Tips = "鑫精诚多通道压力600F";
            this.Icon = "\xe652";
        }
            ...
            if (PActionType == PressureActionType.Clear)
            {
                if (!IsEmptyMode)
                {
                    lock (lockRW)
                    {
                        ...
                        for (int i = 0; i < 1; i++)
                        {
                            bool comResult = communcation.WriteWithoutBlocking<float>((float)(ChannelNo - 1), $"{StationNum} 10 17924 1");
                        }
                    }
                }
            }

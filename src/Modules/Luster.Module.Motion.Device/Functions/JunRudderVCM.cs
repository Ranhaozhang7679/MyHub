...
namespace Luster.Module.Motion.Device.Functions
{
    ...
    public class JunRudderVCM : MotionFunction, IPauseFunction, IStopFunction, INote
    {
        ...
        /// <summary>
        /// 读取0x6077电流反馈，直接用标定系数换算为压力(N)
        /// 压力 = CurrentPressureK × rawValue + CurrentPressureB
        /// </summary>
        private double ReadFeedbackPressure()
        {
            _axis.SDORead(0x6077, 0, 2, out int rawValue, 1);
            return rawValue * CurrentPressureK + CurrentPressureB;
        }

        /// <summary>
        /// 根据目标压力(N)反算扭矩指令
        /// 压力 = TorquePressureK × 扭矩 + TorquePressureB
        /// 扭矩 = (目标压力 - TorquePressureB) / TorquePressureK
        /// </summary>
        private int CalcTorqueFromPressure(double pressure)
        {
            return (int)Math.Round((pressure - TorquePressureB) / TorquePressureK);
        }

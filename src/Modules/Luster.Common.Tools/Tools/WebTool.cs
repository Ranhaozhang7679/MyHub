...
* 类 名 称:       WebTool
* 机器名称:       L05123-NB
* 命名空间:       Luster.Common.Tools.Web
* 文 件 名:       WebTool.cs
* 创建时间:       2022/9/14 9:43:25
* 作    者:       L05123
* 所属部门：      系统集成部
...
* 唯一标识：      ac9c8269-094d-4bea-88b9-22da574522c9
* 登录用户:       darkliu
* 所 属 域:       LUSTERINC
* 创建年份:       2022
* 修改时间:		  2022/9/14 9:43:25
* 修 改 人:		  L05123
************************************************************************************/
#endregion
...
namespace Luster.Common.Tools
{
    ...
    public class WebTool
    {
        ...
        /// <returns></returns>
        public string GetRemoveChar(string url)
        {
            return url.Replace("\n", "").Replace("\r", "");
        }
        ...
        public Task<string> PostAsync(string url, string postData, string contentType = "application/json")
        {
            ...
            request.Timeout = this.Timeout; // 保持现有超时策略
            var dataBytes = this.Encoding != null ? this.Encoding.GetBytes(postData) : Encoding.UTF8.GetBytes(postData);

            return PostAsyncCore(request, dataBytes);
        }

        public Task<string> PostAsync(string url, byte[] postData, string contentType = "application/json")
        {
            if (url == null) throw new ArgumentNullException(nameof(url));
            if (postData == null) postData = Array.Empty<byte>();

            var request = CreateRequest(url, "POST");
            request.ContentType = contentType;
            request.Timeout = this.Timeout;

            return PostAsyncCore(request, postData);
        }

        private async Task<string> PostAsyncCore(HttpWebRequest request, byte[] postData)
        {
            // 写入请求体（异步）
            using (var reqStream = await request.GetRequestStreamAsync().ConfigureAwait(false))
            {
                await reqStream.WriteAsync(postData, 0, postData.Length).ConfigureAwait(false);
            }
            ...
            {
                await respStream.CopyToAsync(ms).ConfigureAwait(false);
                var bytes = ms.ToArray();
                return (this.Encoding ?? Encoding.UTF8).GetString(bytes);
            }
        }
        ...
        /// <returns></returns>    
        public string Post(string url, byte[] postData, string contentType = "application/json")
        {

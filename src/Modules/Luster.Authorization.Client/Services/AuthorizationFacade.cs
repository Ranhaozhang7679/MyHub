...
namespace DC.Authorization.WPF.Services
{
    ...
    public class AuthorizationFacade : IAuthorizationFacade
    {
        ...
        public bool CheckAuth(AuthItem authItem)
        {
            ...
            var auth = _rightRepository.HasRight(_loginService.Current.Id, authItem.Module, authItem.View, authItem.Operation);
            if (!auth)
            {
        ...
        public bool HasAuth(AuthItem authItem)
        {
            if (!_loginService.HasLogin) return false;
            if (_loginService.Current!.IsAdmin) return true;
            if (string.IsNullOrEmpty(authItem.Operation)) return true;

            return _rightRepository.HasRight(_loginService.Current.Id, authItem.Module, authItem.View, authItem.Operation);
        }

...
namespace Luster.Module.Motion.Business.Functions
{
    ...
    /// 工单查询
    /// </summary>
    public class FX_BindCarrier : MotionFunction, IPauseFunction
    {
        public FX_BindCarrier()
        {
            this.Tips = "SFC绑定载具";
            this.Icon = "\xe6a8";
        }
        ...
        [Parameter("SN编码", 2, CN = "SN编码", CanRef = ParamRef.Ref)]
        ...
        [Parameter("通过扫描治具得到的二维码", 11, CN = "治具码", CanRef = ParamRef.Ref)]
        ...
        public override bool DoExcute(out string errMsg)
        {
            ...
            _sfcHelper.BindCarrier(CarrierSN, OutWIP, out errMsg);
            if (!string.IsNullOrEmpty(errMsg))
            {
                // 解绑失败弹出提示信息
                MyOwner.OnAlarm(AlarmType.InfoTip, errMsg);
            ...
            //MyOwner.OnLog(LogType.Debug, $"PDCA发送数据,:{sendStr}");
            ...
            /////绑定治具
            //sendStr = $"sfc_post@c =QUERY_4_SFC&subcmd=carrier_link_csa&carrier_no={CarrierSN}&link_inf=1:{OutWIP}&station_id={StationID}\n";
            //vCommPdca.Write(sendStr);
            //MyOwner.OnLog(LogType.Debug, $"PDCA发送数据，绑定载具:{sendStr}");
            //// 等待结果
            //recStr = vCommPdca.ReadSingle<string>("", 5000);
            //MyOwner.OnLog(LogType.Debug, $"PDCA接收数据,绑定载具:{recStr}");
